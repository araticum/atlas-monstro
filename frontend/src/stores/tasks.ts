...
async function findAssignees(
	parsedTaskAssignees: string[],
	projectId: number,
): Promise<MatchedAssignee[]> {
	...
	const assignees = parsedTaskAssignees.map(async (a) => {
		const users = (await userService.getAll({ projectId }, { s: a })).map(
			(u) => ({
				...u,
				match: a,
			}),
...
export const useTaskStore = defineStore('task', () => {
	...
	function addTaskAttachment({
		...
		if (t.task !== null) {
			const attachments = [...t.task.attachments, attachment]
	...
	async function addAssignee({
		...
		try {
			...
			const r = await taskAssigneeService.create(
			...
			if (t.task === null) {
				...
				console.debug(
					'Could not add assignee to task in kanban, task not found',
					t,
				)
				return r
			}
			...
			kanbanStore.setTaskInBucketByIndex({
				...
				task: {
					...t.task,
					assignees: [...t.task.assignees, user],
				},
			})
	...
	async function removeAssignee({
		...
		const taskAssigneeService = new TaskAssigneeService()
		const response = await taskAssigneeService.delete(
			new TaskAssigneeModel({
				userId: user.id,
				taskId: taskId,
			}),
		...
		if (t.task === null) {
			...
			// Vuex seems to have its difficulties with that, so we just log the error and fail silently.
			console.debug(
				'Could not remove assignee from task in kanban, task not found',
				t,
			)
			return response
		}
	...
	async function addLabel({
		...
		const labelTaskService = new LabelTaskService()
		const r = await labelTaskService.create(
			new LabelTaskModel({
				taskId,
				labelId: label.id,
			}),
		...
		if (t.task === null) {
			...
			// Usually this means the kanban board hasn't been accessed until now.
			// Vuex seems to have its difficulties with that, so we just log the error and fail silently.
			console.debug('Could not add label to task in kanban, task not found', {
				taskId,
				t,
			})
			return r
	...
	async function removeLabel({
		...
		const labelTaskService = new LabelTaskService()
		const response = await labelTaskService.delete(
			new LabelTaskModel({
				taskId,
				labelId: label.id,
			}),
		...
		if (t.task === null) {
			...
			// Vuex seems to have its difficulties with that, so we just log the error and fail silently.
			console.debug(
				'Could not remove label from task in kanban, task not found',
				t,
			)
			return response
		}
	...
	function findProjectId({
		...
		const projectIdFromRoute = Number(
			router.currentRoute.value.params.projectId,
		)
		if (
			typeof router.currentRoute.value.params.projectId !== 'undefined' &&
			projectIdFromRoute > 0
		) {
	...
	async function createNewTask({
		...
		const cancel = setModuleLoading(setIsLoading)
		const quickAddMagicMode =
			authStore.settings.frontendSettings.quickAddMagicMode
		...
		const dueDate =
			parsedTask.date !== null ? new Date(parsedTask.date).toISOString() : null
		...
		if (
			parsedTask.repeats?.type === REPEAT_TYPES.Months &&
			parsedTask.repeats?.amount === 1
		) {
	...
	async function duplicateTask(taskId: ITask['id']) {
		...
		try {
			...
			const response = await taskDuplicateService.create(
				new TaskDuplicateModel({ taskId }),
			)
	...
	async function setTaskTemplate(taskId: ITask['id'], isTemplate: boolean) {
		const cancel = setModuleLoading(setIsLoading)
		try {
			const service = new TaskTemplateService()
			return await service.setIsTemplate(taskId, isTemplate)
		} finally {
			cancel()
		}
	}
